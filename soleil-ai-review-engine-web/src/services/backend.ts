/**
 * Stateless HTTP client for the local soleil-ai-review-engine backend server.
...
const fetchWithTimeout = async (
  ...
  try {
    ...
    if (error instanceof TypeError) {
      throw new Error(`Network error reaching soleil-ai-review-engine backend at ${backendUrl}: ${error.message}`);
    }
    throw error;
  } finally {
    clearTimeout(timer);
