...
export const CodeReferencesPanel = ({ onFocusNode }: CodeReferencesPanelProps) => {
  ...
  const [panelWidth, setPanelWidth] = useState<number>(() => {
    try {
      const saved = window.localStorage.getItem('soleil-ai-review-engine.codePanelWidth');
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem('soleil-ai-review-engine.codePanelWidth', String(panelWidth));
    } catch {
      // ignore
    }
  }, [panelWidth]);
