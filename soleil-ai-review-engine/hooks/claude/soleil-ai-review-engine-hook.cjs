#!/usr/bin/env node
/**
 * soleil-ai-review-engine Claude Code Hook
 *
 * PreToolUse  — intercepts Grep/Glob/Bash searches and augments
 *               with graph context from the soleil-ai-review-engine index.
...
/**
 * Find the .soleil-ai-review-engine directory by walking up from startDir.
 * Returns the path to .soleil-ai-review-engine/ or null if not found.
 */
function findSoleilDir(startDir) {
  let dir = startDir || process.cwd();
  for (let i = 0; i < 5; i++) {
    const candidate = path.join(dir, '.soleil-ai-review-engine');
...
/**
 * Resolve the soleil-ai-review-engine CLI path.
 * 1. Relative path (works when script is inside npm package)
 * 2. require.resolve (works when soleil-ai-review-engine is globally installed)
...
function resolveCliPath() {
  ...
  if (!fs.existsSync(cliPath)) {
    try {
      cliPath = require.resolve('soleil-ai-review-engine/dist/cli/index.js');
    } catch {
      cliPath = '';
    }
  }
...
function runsoleil-ai-review-engineCli(cliPath, args, cwd, timeout) {
  ...
  return spawnSync(
    isWin ? 'npx.cmd' : 'npx',
    ['-y', 'soleil-ai-review-engine', ...args],
 ...
 *
 * Instead of spawning a full `soleil-ai-review-engine analyze` synchronously (which blocks
 * the agent for up to 120s and risks KuzuDB corruption on timeout), we do a
 * lightweight staleness check: compare `git rev-parse HEAD` against the
 * lastCommit stored in `.soleil-ai-review-engine/meta.json`. If they differ, notify the
...
function handlePostToolUse(input) {
  ...
  const cwd = input.cwd || process.cwd();
  if (!path.isAbsolute(cwd)) return;
  const soleilDir = findSoleilDir(cwd);
  if (!soleilDir) return;
  ...
  try {
    const meta = JSON.parse(fs.readFileSync(path.join(soleilDir, 'meta.json'), 'utf-8'));
...
function main() {
  try {
    ...
    if (process.env.SOLEIL_AI_REVIEW_ENGINE_DEBUG) {
      console.error('soleil-ai-review-engine hook error:', (err.message || '').slice(0, 200));
    }
  }
}
