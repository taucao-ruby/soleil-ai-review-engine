#!/bin/bash
# soleil-ai-review-engine PreToolUse hook for Claude Code
...
# Check if we're in a soleil-ai-review-engine-indexed repo
dir="${CWD:-$PWD}"
found=false
...
# Run soleil-ai-review-engine augment — must be fast (<500ms target)
# augment writes to stderr (KuzuDB captures stdout at OS level), so capture stderr and discard stdout
RESULT=$(cd "$CWD" && npx -y soleil-ai-review-engine augment "$PATTERN" 2>&1 1>/dev/null)
