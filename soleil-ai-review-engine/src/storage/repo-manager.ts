 ...
 * Repository Manager
 * 
 * Manages soleil-ai-review-engine index storage in .soleil-ai-review-engine/ at repo root.
 * Also maintains a global registry at ~/.soleil-ai-review-engine/registry.json
...
/**
 * Shape of an entry in the global registry (~/.soleil-ai-review-engine/registry.json)
 */
...
const SOLEIL_AI_REVIEW_ENGINE_DIR = '.soleil-ai-review-engine';

// ─── Local Storage Helpers ─────────────────────────────────────────────

/**
 * Get the .soleil-ai-review-engine storage path for a repository
 */
export const getStoragePath = (repoPath: string): string => {
  return path.join(path.resolve(repoPath), SOLEIL_AI_REVIEW_ENGINE_DIR);
};
 ...
 *
 * Returns:
 *   found        — true if .soleil-ai-review-engine/kuzu existed and was deleted
...
/**
 * Check if a path has a soleil-ai-review-engine index
 */
...
/**
 * Find .soleil-ai-review-engine by walking up from a starting path
 */
...
/**
 * Add .soleil-ai-review-engine to .gitignore if not already present
 */
export const addToGitignore = async (repoPath: string): Promise<void> => {
  ...
  try {
    ...
    if (content.includes(SOLEIL_AI_REVIEW_ENGINE_DIR)) return;
    
    const newContent = content.endsWith('\n') 
      ? `${content}${SOLEIL_AI_REVIEW_ENGINE_DIR}\n`
      : `${content}\n${SOLEIL_AI_REVIEW_ENGINE_DIR}\n`;
    await fs.writeFile(gitignorePath, newContent, 'utf-8');
  } catch {
    // .gitignore doesn't exist, create it
    await fs.writeFile(gitignorePath, `${SOLEIL_AI_REVIEW_ENGINE_DIR}\n`, 'utf-8');
  }
};

// ─── Global Registry (~/.soleil-ai-review-engine/registry.json) ───────────────────────

/**
 * Get the path to the global soleil-ai-review-engine directory
 */
export const getGlobalDir = (): string => {
  return path.join(os.homedir(), '.soleil-ai-review-engine');
};
 ...
 * Called after `soleil-ai-review-engine analyze` completes.
 */
 ...
 * Called after `soleil-ai-review-engine clean`.
 */
 ...
 * Optionally validates that each entry's .soleil-ai-review-engine/ still exists.
 */
export const listRegisteredRepos = async (opts?: { validate?: boolean }): Promise<RegistryEntry[]> => {
  const entries = await readRegistry();
  if (!opts?.validate) return entries;

  // Validate each entry still has a .soleil-ai-review-engine/ directory
...
// ─── Global CLI Config (~/.soleil-ai-review-engine/config.json) ─────────────────────────
...
/**
 * Load CLI config from ~/.soleil-ai-review-engine/config.json
 */
...
/**
 * Save CLI config to ~/.soleil-ai-review-engine/config.json
 */
