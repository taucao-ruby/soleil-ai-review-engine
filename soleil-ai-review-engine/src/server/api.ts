 ...
 * HTTP API Server
 *
 * REST API for browser-based clients to query the local .soleil-ai-review-engine/ index.
...
export const createServer = async (port: number, host: string = '127.0.0.1') => {
  ...
  app.use(cors({
    origin: (origin, callback) => {
      if (
        ...
        || origin === 'https://soleil-ai-review-engine.vercel.app'
      ) {
        callback(null, true);
      } else {
  ...
  app.get('/api/repo', async (req, res) => {
    try {
      ...
      if (!entry) {
        res.status(404).json({ error: 'Repository not found. Run: soleil-ai-review-engine analyze' });
        return;
      }
  ...
  const server = app.listen(port, host, () => {
    console.log(`soleil-ai-review-engine server running on http://${host}:${port}`);
  });
