...
async function findRepoForCwd(cwd: string): Promise<{
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      // (e.g. /projects/soleil-ai-review-enginev2 should NOT match /projects/soleil-ai-review-engine)
      let matched = false;
...
export async function augment(pattern: string, cwd?: string): Promise<string> {
  ...
  try {
    ...
    const lines: string[] = [`[soleil-ai-review-engine] ${enriched.length} related symbols found:`, ''];
