 ...
 * Clean Command
 * 
 * Removes the .soleil-ai-review-engine index from the current repository.
...
export const cleanCommand = async (options?: { force?: boolean; all?: boolean }) => {
  ...
  if (options?.all) {
    if (!options?.force) {
      ...
      console.log(`This will delete soleil-ai-review-engine indexes for ${entries.length} repo(s):`);
  ...
  if (!options?.force) {
    console.log(`This will delete the soleil-ai-review-engine index for: ${repoName}`);
