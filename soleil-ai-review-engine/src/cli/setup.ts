 ...
 * so the soleil-ai-review-engine MCP server is available in all projects.
 */
...
/**
 * Merge soleil-ai-review-engine entry into an existing MCP config JSON object.
...
/**
 * Install soleil-ai-review-engine skills to ~/.claude/skills/ for Claude Code.
 */
...
/**
 * Install soleil-ai-review-engine hooks to ~/.claude/settings.json for Claude Code.
...
async function installClaudeCodeHooks(result: SetupResult): Promise<void> {
  ...
  // Source hooks bundled within the soleil-ai-review-engine package (hooks/claude/)
  const pluginHooksPath = path.join(__dirname, '..', '..', 'hooks', 'claude');

  // Copy unified hook script to ~/.claude/hooks/soleil-ai-review-engine/
  const destHooksDir = path.join(claudeDir, 'hooks', 'soleil-ai-review-engine');

  try {
    await fs.mkdir(destHooksDir, { recursive: true });

    const src = path.join(pluginHooksPath, 'soleil-ai-review-engine-hook.cjs');
    const dest = path.join(destHooksDir, 'soleil-ai-review-engine-hook.cjs');
    try {
    ...
    const hookPath = path.join(destHooksDir, 'soleil-ai-review-engine-hook.cjs').replace(/\\/g, '/');
    ...
    // Helper: add a hook entry if one with 'soleil-ai-review-engine-hook' isn't already registered
    ...
    function ensureHookEntry(
      ...
      const hasHook = existing.hooks[eventName].some(
        (h: HookEntry) => h.hooks?.some(hh => hh.command?.includes('soleil-ai-review-engine-hook'))
      );
      if (!hasHook) {
    ...
    ensureHookEntry('PreToolUse', 'Grep|Glob|Bash', 10, 'Enriching with soleil-ai-review-engine graph context...');
    ensureHookEntry('PostToolUse', 'Bash', 10, 'Checking soleil-ai-review-engine index freshness...');
...
/**
 * Install soleil-ai-review-engine skills to a target directory.
 * Each skill is installed as {targetDir}/soleil-ai-review-engine-{skillName}/SKILL.md
...
/**
 * Install global Cursor skills to ~/.cursor/skills/soleil-ai-review-engine/
 */
...
/**
 * Install global OpenCode skills to ~/.config/opencode/skill/soleil-ai-review-engine/
 */
...
export const setupCommand = async () => {
  console.log('');
  console.log('  soleil-ai-review-engine Setup');
  console.log('  ==============');
  console.log('');
