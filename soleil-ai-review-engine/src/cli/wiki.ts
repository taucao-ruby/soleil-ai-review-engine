 ...
 * Usage: soleil-ai-review-engine wiki [path] [options]
 */
...
export const wikiCommand = async (
  ...
  console.log('\n  soleil-ai-review-engine Wiki Generator\n');
  ...
  if (!meta) {
    console.log('  Error: No soleil-ai-review-engine index found.');
    console.log('  Run `soleil-ai-review-engine analyze` first to index this repository.\n');
    process.exitCode = 1;
    return;
  }
  ...
  if (options?.apiKey || options?.model || options?.baseUrl) {
    ...
    console.log('  Config saved to ~/.soleil-ai-review-engine/config.json\n');
  }
  ...
  if (!hasSavedConfig && !hasCLIOverrides) {
    if (!process.stdin.isTTY) {
      if (!llmConfig.apiKey) {
        ...
        console.log('  Set OPENAI_API_KEY or SOLEIL_AI_REVIEW_ENGINE_API_KEY environment variable,');
      ...
      const envKey = process.env.SOLEIL_AI_REVIEW_ENGINE_API_KEY || process.env.OPENAI_API_KEY || '';
      let key: string;
      if (envKey) {
    ...
    if (err.message?.includes('No source files')) {
      ...
      if (isAuthError && process.stdin.isTTY) {
        ...
        if (!answer || answer === 'y' || answer === 'yes') {
          ...
          await saveCLIConfig({});
          console.log('  Config cleared. Run `soleil-ai-review-engine wiki` again to reconfigure.\n');
        }
      }
    } else {
      console.log(`\n  Error: ${err.message}\n`);
...
function publishGist(htmlPath: string): { url: string; rawUrl: string } | null {
  try {
    const output = execFileSync('gh', [
      ...
      '--desc', 'Repository Wiki — generated by soleil-ai-review-engine',
      '--public',
