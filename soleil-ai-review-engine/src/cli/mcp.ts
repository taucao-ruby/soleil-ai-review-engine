...
export const mcpCommand = async () => {
  ...
  process.on('uncaughtException', (err) => {
    console.error(`soleil-ai-review-engine MCP: uncaught exception — ${err.message}`);
    // Process is in an undefined state after uncaughtException — exit after flushing
    setTimeout(() => process.exit(1), 100);
  });
  process.on('unhandledRejection', (reason) => {
    const msg = reason instanceof Error ? reason.message : String(reason);
    console.error(`soleil-ai-review-engine MCP: unhandled rejection — ${msg}`);
  });
  ...
  if (repos.length === 0) {
    console.error('soleil-ai-review-engine: No indexed repos yet. Run `soleil-ai-review-engine analyze` in a git repo — the server will pick it up automatically.');
  } else {
    console.error(`soleil-ai-review-engine: MCP server starting with ${repos.length} repo(s): ${repos.map(r => r.name).join(', ')}`);
  }
