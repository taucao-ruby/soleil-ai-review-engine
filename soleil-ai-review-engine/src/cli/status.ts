...
export const statusCommand = async () => {
  ...
  if (!repo) {
    ...
    if (await hasKuzuIndex(storagePath)) {
      ...
      console.log('Run: soleil-ai-review-engine analyze   (rebuilds the index with LadybugDB)');
    } else {
      console.log('Repository not indexed.');
      console.log('Run: soleil-ai-review-engine analyze');
    }
    return;
  }
  ...
  console.log(`Status: ${isUpToDate ? '✅ up-to-date' : '⚠️ stale (re-run soleil-ai-review-engine analyze)'}`);
};
