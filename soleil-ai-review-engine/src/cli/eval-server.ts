 ...
 * 
 * Usage:
 *   soleil-ai-review-engine eval-server                    # default port 4848
 *   soleil-ai-review-engine eval-server --port 4848        # explicit port
 *   soleil-ai-review-engine eval-server --idle-timeout 300 # auto-shutdown after 300s idle
 * 
 * API:
...
export function formatContextResult(result: any): string {
  ...
  if (result.status === 'ambiguous') {
    ...
    lines.push(`\nRe-run: soleil-ai-review-engine-context "${result.candidates?.[0]?.name}" "<file_path>"`);
    return lines.join('\n');
  }
...
function getNextStepHint(toolName: string): string {
  switch (toolName) {
    case 'query':
      return '\n---\nNext: Pick a symbol above and run soleil-ai-review-engine-context "<name>" to see all its callers, callees, and execution flows.';

    case 'context':
      return '\n---\nNext: To check what breaks if you change this, run soleil-ai-review-engine-impact "<name>" upstream';

    case 'impact':
      return '\n---\nNext: Review d=1 items first (WILL BREAK). Read the source with cat to understand the code, then make your fix.';

    case 'cypher':
      return '\n---\nNext: To explore a result symbol in depth, run soleil-ai-review-engine-context "<name>"';

    case 'detect_changes':
      return '\n---\nNext: Run soleil-ai-review-engine-context "<symbol>" on high-risk changed symbols to check their callers.';
...
export async function evalServerCommand(options?: EvalServerOptions): Promise<void> {
  ...
  if (!ok) {
    console.error('soleil-ai-review-engine eval-server: No indexed repositories found. Run: soleil-ai-review-engine analyze');
    process.exit(1);
  }

  const repos = await backend.listRepos();
  console.error(`soleil-ai-review-engine eval-server: ${repos.length} repo(s) loaded: ${repos.map(r => r.name).join(', ')}`);
  ...
  function resetIdleTimer() {
    ...
    idleTimer = setTimeout(async () => {
      console.error('soleil-ai-review-engine eval-server: Idle timeout reached, shutting down');
  ...
  server.listen(port, '127.0.0.1', () => {
    console.error(`soleil-ai-review-engine eval-server: listening on http://127.0.0.1:${port}`);
    ...
    try {
      process.stdout.write(`SOLEIL_AI_REVIEW_ENGINE_EVAL_SERVER_READY:${port}\n`);
    } catch {
  ...
  const shutdown = async () => {
    console.error('soleil-ai-review-engine eval-server: shutting down...');
    await backend.disconnect();
    server.close();
    process.exit(0);
  };
