...
// NOTE: Negation patterns in .soleil-ai-review-engineignore (e.g. `!vendor/`) cannot override
...
/**
 * Load .gitignore and .soleil-ai-review-engineignore rules from the repo root.
 * Returns an `ignore` instance with all patterns, or null if no files found.
 */
export interface IgnoreOptions {
  /** Skip .gitignore parsing, only read .soleil-ai-review-engineignore. Defaults to SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE env var. */
  noGitignore?: boolean;
}
...
export const loadIgnoreRules = async (
  ...
  const skipGitignore = options?.noGitignore ?? !!process.env.SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE;
  const filenames = skipGitignore
    ? ['.soleil-ai-review-engineignore']
    : ['.gitignore', '.soleil-ai-review-engineignore'];
 ...
 * - .gitignore / .soleil-ai-review-engineignore patterns (via `ignore` package)
...
export const createIgnoreFilter = async (repoPath: string, options?: IgnoreOptions) => {
  ...
  return {
    ignored(p: Path): boolean {
      ...
      if (!rel) return false;
      // Check .gitignore / .soleil-ai-review-engineignore patterns
    ...
    childrenIgnored(p: Path): boolean {
      ...
      // Check against .gitignore / .soleil-ai-review-engineignore patterns.
