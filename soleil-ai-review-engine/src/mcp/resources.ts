 ...
 * All resources use repo-scoped URIs: soleil-ai-review-engine://repo/{name}/context
 */
...
export function getResourceDefinitions(): ResourceDefinition[] {
  return [
    {
      uri: 'soleil-ai-review-engine://repos',
    ...
    {
      uri: 'soleil-ai-review-engine://setup',
      name: 'soleil-ai-review-engine Setup Content',
...
export function getResourceTemplates(): ResourceTemplate[] {
  return [
    {
      uriTemplate: 'soleil-ai-review-engine://repo/{name}/context',
      name: 'Repo Overview',
      description: 'Codebase stats, staleness check, and available tools',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'soleil-ai-review-engine://repo/{name}/clusters',
      name: 'Repo Modules',
      description: 'All functional areas (Leiden clusters)',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'soleil-ai-review-engine://repo/{name}/processes',
      name: 'Repo Processes',
      description: 'All execution flows',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'soleil-ai-review-engine://repo/{name}/schema',
      name: 'Graph Schema',
      description: 'Node/edge schema for Cypher queries',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'soleil-ai-review-engine://repo/{name}/cluster/{clusterName}',
      name: 'Module Detail',
      description: 'Deep dive into a specific functional area',
      mimeType: 'text/yaml',
    },
    {
      uriTemplate: 'soleil-ai-review-engine://repo/{name}/process/{processName}',
      name: 'Process Trace',
...
function parseUri(uri: string): { repoName?: string; resourceType: string; param?: string } {
  if (uri === 'soleil-ai-review-engine://repos') return { resourceType: 'repos' };
  if (uri === 'soleil-ai-review-engine://setup') return { resourceType: 'setup' };

  // Repo-scoped: soleil-ai-review-engine://repo/{name}/context
  const repoMatch = uri.match(/^soleil-ai-review-engine:\/\/repo\/([^/]+)\/(.+)$/);
  if (repoMatch) {
...
async function getReposResource(backend: LocalBackend): Promise<string> {
  ...
  if (repos.length === 0) {
    return 'repos: []\n# No repositories indexed. Run: soleil-ai-review-engine analyze';
  }
  ...
  if (repos.length > 1) {
    ...
    lines.push(`# soleil-ai-review-engine_search({query: "auth", repo: "${repos[0].name}"})`);
  }
...
async function getContextResource(backend: LocalBackend, repoName?: string): Promise<string> {
  ...
  if (!context) {
    return 'error: No codebase loaded. Run: soleil-ai-review-engine analyze';
  }
  ...
  lines.push(`  - soleil-ai-review-engine://repo/${context.projectName}/clusters: All functional areas`);
  lines.push(`  - soleil-ai-review-engine://repo/${context.projectName}/processes: All execution flows`);
  lines.push(`  - soleil-ai-review-engine://repo/${context.projectName}/cluster/{name}: Module details`);
  lines.push(`  - soleil-ai-review-engine://repo/${context.projectName}/process/{name}: Process trace`);
...
async function getClustersResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.clusters || result.clusters.length === 0) {
      return 'modules: []\n# No functional areas detected. Run: soleil-ai-review-engine analyze';
    }
    ...
    if (result.clusters.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.clusters.length} modules. Use soleil-ai-review-engine_query for deeper search.`);
    }
...
async function getProcessesResource(backend: LocalBackend, repoName?: string): Promise<string> {
  try {
    ...
    if (!result.processes || result.processes.length === 0) {
      return 'processes: []\n# No processes detected. Run: soleil-ai-review-engine analyze';
    }
    ...
    if (result.processes.length > displayLimit) {
      lines.push(`\n# Showing top ${displayLimit} of ${result.processes.length} processes. Use soleil-ai-review-engine_query for deeper search.`);
    }
...
function getSchemaResource(): string {
  return `# soleil-ai-review-engine Graph Schema
 ...
 * Useful for `soleil-ai-review-engine setup` onboarding or dynamic content injection.
 */
async function getSetupResource(backend: LocalBackend): Promise<string> {
  ...
  for (const repo of repos) {
    ...
    const lines = [
      `# soleil-ai-review-engine MCP — ${repo.name}`,
      '',
      `This project is indexed by soleil-ai-review-engine as **${repo.name}** (${stats.nodes || 0} symbols, ${stats.edges || 0} relationships, ${stats.processes || 0} execution flows).`,
      '',
      '## Tools',
      '',
      ...
      '## Resources',
      '',
      `- \`soleil-ai-review-engine://repo/${repo.name}/context\` — Stats, staleness check`,
      `- \`soleil-ai-review-engine://repo/${repo.name}/clusters\` — All functional areas`,
      `- \`soleil-ai-review-engine://repo/${repo.name}/processes\` — All execution flows`,
      `- \`soleil-ai-review-engine://repo/${repo.name}/schema\` — Graph schema for Cypher`,
    ];
