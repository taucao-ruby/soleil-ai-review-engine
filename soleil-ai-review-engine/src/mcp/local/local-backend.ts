 ...
 * 
 * Provides tool implementations using local .soleil-ai-review-engine/ indexes.
...
// AI context generation is CLI-only (soleil-ai-review-engine analyze)
...
function logQueryError(context: string, err: unknown): void {
  ...
  console.error(`soleil-ai-review-engine [${context}]: ${msg}`);
}
...
export class LocalBackend {
  ...
  private async refreshRepos(): Promise<void> {
    ...
    for (const entry of entries) {
      ...
      if (kuzu.found && kuzu.needsReindex) {
        console.error(`soleil-ai-review-engine: "${entry.name}" has a stale KuzuDB index. Run: soleil-ai-review-engine analyze ${entry.path}`);
      }
  ...
  async resolveRepo(repoParam?: string): Promise<RepoHandle> {
    ...
    if (this.repos.size === 0) {
      throw new Error('No indexed repositories. Run: soleil-ai-review-engine analyze');
    }
    if (repoParam) {
  ...
  private async bm25Search(repo: RepoHandle, query: string, limit: number): Promise<any[]> {
    ...
    try {
      ...
      console.error('soleil-ai-review-engine: BM25/FTS search failed (FTS indexes may not exist) -', err.message);
      return [];
    }
