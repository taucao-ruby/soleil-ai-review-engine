...
export const initEmbedder = async (): Promise<FeatureExtractionPipeline> => {
  ...
  initPromise = (async () => {
    try {
      ...
      console.error('soleil-ai-review-engine: Loading embedding model (first search may take a moment)...');
      ...
      for (const device of devicesToTry) {
        try {
          ...
          console.error(`soleil-ai-review-engine: Embedding model loaded (${device})`);
          return embedderInstance!;
        } catch {
