...
describe('filesystem-walker', () => {
  ...
  describe('.soleil-ai-review-engineignore support', () => {
    ...
    beforeAll(async () => {
      ...
      // Only .soleil-ai-review-engineignore, no .gitignore
      await fs.writeFile(path.join(nexusignoreDir, '.soleil-ai-review-engineignore'), 'local/\n');
    });
    ...
    it('excludes directories listed in .soleil-ai-review-engineignore', async () => {
  ...
  describe('combined .gitignore + .soleil-ai-review-engineignore', () => {
    ...
    beforeAll(async () => {
      ...
      await fs.writeFile(path.join(combinedDir, '.soleil-ai-review-engineignore'), 'local/\n');
    });
  ...
  describe('SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE env var', () => {
    let envDir: string;
    ...
    it('includes gitignored directory when SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE is set', async () => {
      const original = process.env.SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE;
      process.env.SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE = '1';
      try {
        const files = await walkRepositoryPaths(envDir);
        const paths = files.map(f => f.path.replace(/\\/g, '/'));
        ...
        if (original === undefined) {
          delete process.env.SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE;
        } else {
          process.env.SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE = original;
        }
      }
    });
  });
