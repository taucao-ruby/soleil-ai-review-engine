 ...
 *
 * Tests the hook scripts with real git repos and .soleil-ai-review-engine directories.
...
const CJS_HOOK = path.resolve(__dirname, '..', '..', 'hooks', 'claude', 'soleil-ai-review-engine-hook.cjs');
const PLUGIN_HOOK = path.resolve(__dirname, '..', '..', '..', 'soleil-ai-review-engine-claude-plugin', 'hooks', 'soleil-ai-review-engine-hook.js');
...
// ─── Temp git repo with .soleil-ai-review-engine ───────────────────────────────────

let tmpDir: string;
let soleilDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hooks-e2e-'));
  soleilDir = path.join(tmpDir, '.soleil-ai-review-engine');
  fs.mkdirSync(soleilDir, { recursive: true });
...
describe.each(HOOKS)('hooks e2e ($name)', ({ name, path: hookPath }) => {
  describe('PostToolUse staleness detection', () => {
    ...
    it('treats missing meta.json as stale', () => {
      // Remove meta.json
      const metaPath = path.join(soleilDir, 'meta.json');
  ...
  describe('PreToolUse — silent without soleil-ai-review-engine CLI', () => {
    // PreToolUse tries to spawn `soleil-ai-review-engine augment` which won't be available in CI.
    ...
    it('handles Grep pattern gracefully when CLI is unavailable', () => {
      ...
      // spawned `soleil-ai-review-engine augment` hangs and the 10s timeout kills the process.
  ...
  describe('directory without .soleil-ai-review-engine', () => {
    ...
    it('ignores PostToolUse when no .soleil-ai-review-engine directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PostToolUse',
        tool_name: 'Bash',
        tool_input: { command: 'git commit -m "x"' },
    ...
    it('ignores PreToolUse when no .soleil-ai-review-engine directory exists', () => {
      const result = runHook(hookPath, {
        hook_event_name: 'PreToolUse',
        tool_name: 'Grep',
