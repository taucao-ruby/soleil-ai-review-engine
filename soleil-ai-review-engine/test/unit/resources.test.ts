...
describe('getResourceDefinitions', () => {
  ...
  it('includes repos resource', () => {
    const defs = getResourceDefinitions();
    const repos = defs.find(d => d.uri === 'soleil-ai-review-engine://repos');
  ...
  it('includes setup resource', () => {
    const defs = getResourceDefinitions();
    const setup = defs.find(d => d.uri === 'soleil-ai-review-engine://setup');
...
describe('getResourceTemplates', () => {
  ...
  it('includes context, clusters, processes, schema, cluster detail, process detail', () => {
    ...
    expect(uris).toContain('soleil-ai-review-engine://repo/{name}/context');
    expect(uris).toContain('soleil-ai-review-engine://repo/{name}/clusters');
    expect(uris).toContain('soleil-ai-review-engine://repo/{name}/processes');
    expect(uris).toContain('soleil-ai-review-engine://repo/{name}/schema');
    expect(uris).toContain('soleil-ai-review-engine://repo/{name}/cluster/{clusterName}');
    expect(uris).toContain('soleil-ai-review-engine://repo/{name}/process/{processName}');
  });
...
describe('readResource', () => {
  it('routes soleil-ai-review-engine://repos to listRepos', async () => {
    ...
    const result = await readResource('soleil-ai-review-engine://repos', backend);
  ...
  it('routes soleil-ai-review-engine://setup to setup resource', async () => {
    const backend = createMockBackend({
      repos: [
        { name: 'proj', path: '/tmp/proj', indexedAt: '2024-01-01', lastCommit: 'abc', stats: { nodes: 10, edges: 20, processes: 3 } },
      ],
    });
    const result = await readResource('soleil-ai-review-engine://setup', backend);
    expect(result).toContain('soleil-ai-review-engine MCP');
  ...
  it('routes soleil-ai-review-engine://repo/{name}/context correctly', async () => {
    const backend = createMockBackend({
      context: {
    ...
    const result = await readResource('soleil-ai-review-engine://repo/test-project/context', backend);
  ...
  it('routes soleil-ai-review-engine://repo/{name}/schema to static schema', async () => {
    const backend = createMockBackend();
    const result = await readResource('soleil-ai-review-engine://repo/any/schema', backend);
    expect(result).toContain('soleil-ai-review-engine Graph Schema');
    expect(result).toContain('CALLS');
    expect(result).toContain('IMPORTS');
  });

  it('routes soleil-ai-review-engine://repo/{name}/clusters correctly', async () => {
    const backend = createMockBackend({
      clusters: {
        clusters: [
    ...
    const result = await readResource('soleil-ai-review-engine://repo/test/clusters', backend);
  ...
  it('routes soleil-ai-review-engine://repo/{name}/processes correctly', async () => {
    const backend = createMockBackend({
      processes: {
        processes: [
    ...
    const result = await readResource('soleil-ai-review-engine://repo/test/processes', backend);
  ...
  it('routes soleil-ai-review-engine://repo/{name}/cluster/{clusterName} correctly', async () => {
    const backend = createMockBackend({
      clusterDetail: {
    ...
    const result = await readResource('soleil-ai-review-engine://repo/test/cluster/Auth', backend);
  ...
  it('handles cluster detail error', async () => {
    ...
    const result = await readResource('soleil-ai-review-engine://repo/test/cluster/Missing', backend);
    expect(result).toContain('Cluster not found');
  });

  it('routes soleil-ai-review-engine://repo/{name}/process/{processName} correctly', async () => {
    const backend = createMockBackend({
      processDetail: {
    ...
    const result = await readResource('soleil-ai-review-engine://repo/test/process/LoginFlow', backend);
  ...
  it('handles process detail error', async () => {
    ...
    const result = await readResource('soleil-ai-review-engine://repo/test/process/Missing', backend);
    expect(result).toContain('Process not found');
  });

  it('throws for unknown resource URI', async () => {
    const backend = createMockBackend();
    await expect(readResource('soleil-ai-review-engine://unknown', backend))
      .rejects.toThrow('Unknown resource URI');
  });

  it('throws for unknown repo-scoped resource type', async () => {
    const backend = createMockBackend();
    await expect(readResource('soleil-ai-review-engine://repo/test/nonexistent', backend))
      .rejects.toThrow('Unknown resource');
  });

  it('decodes URI-encoded repo names', async () => {
    const backend = createMockBackend();
    await readResource('soleil-ai-review-engine://repo/my%20project/schema', backend);
  ...
  it('decodes URI-encoded cluster names', async () => {
    ...
    await readResource('soleil-ai-review-engine://repo/test/cluster/Auth%20Module', backend);
