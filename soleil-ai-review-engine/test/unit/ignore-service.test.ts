...
describe('loadIgnoreRules', () => {
  ...
  it('parses .soleil-ai-review-engineignore file', async () => {
    await fs.writeFile(path.join(tmpDir, '.soleil-ai-review-engineignore'), 'vendor/\n*.test.ts\n');
    const ig = await loadIgnoreRules(tmpDir);
    expect(ig).not.toBeNull();
    ...
    await fs.unlink(path.join(tmpDir, '.soleil-ai-review-engineignore'));
  });
...
describe('loadIgnoreRules — SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE env var', () => {
  let tmpDir: string;
  ...
  it('skips .gitignore when SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.gitignore'), 'data/\n');
  ...
  it('still reads .soleil-ai-review-engineignore when SOLEIL_AI_REVIEW_ENGINE_NO_GITIGNORE is set', async () => {
    await fs.writeFile(path.join(tmpDir, '.soleil-ai-review-engineignore'), 'vendor/\n');
