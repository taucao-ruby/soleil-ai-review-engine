...
describe('sequential native parser availability', () => {
  ...
  it('warns when processImports skips files in verbose mode', async () => {
    ...
    const previous = process.env.SOLEIL_AI_REVIEW_ENGINE_VERBOSE;
    ...
    if (previous === undefined) {
      delete process.env.SOLEIL_AI_REVIEW_ENGINE_VERBOSE;
    } else {
      process.env.SOLEIL_AI_REVIEW_ENGINE_VERBOSE = previous;
    }
  });
