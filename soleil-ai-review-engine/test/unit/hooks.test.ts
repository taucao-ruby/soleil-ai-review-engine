 ...
 *
 * Tests the hook scripts (soleil-ai-review-engine-hook.cjs and soleil-ai-review-engine-hook.js) that run
 * as PreToolUse and PostToolUse hooks in Claude Code.
 *
 * Covers:
 * - extractPattern: pattern extraction from Grep/Glob/Bash tool inputs
 * - findsoleil-ai-review-engineDir: .soleil-ai-review-engine directory discovery
...
// ─── Test fixtures: temporary .soleil-ai-review-engine directory ───────────────────

let tmpDir: string;
let soleilDir: string;

beforeAll(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'soleil-ai-review-engine-hook-test-'));
...
describe('Windows .cmd extension handling', () => {
  ...
  it('Plugin hook uses .cmd extension for Windows soleil-ai-review-engine binary', () => {
    const source = fs.readFileSync(PLUGIN_HOOK, 'utf-8');
    expect(source).toContain("soleil-ai-review-engine.cmd");
  });
});
