 ...
 *
 * Tests: SOLEIL_AI_REVIEW_ENGINE_TOOLS from tools.ts
...
import { SOLEIL_AI_REVIEW_ENGINE_TOOLS, type ToolDefinition } from '../../src/mcp/tools.js';

describe('SOLEIL_AI_REVIEW_ENGINE_TOOLS', () => {
  it('exports exactly 7 tools', () => {
    expect(SOLEIL_AI_REVIEW_ENGINE_TOOLS).toHaveLength(7);
  });

  it('contains all expected tool names', () => {
    const names = SOLEIL_AI_REVIEW_ENGINE_TOOLS.map(t => t.name);
  ...
  it('each tool has name, description, and inputSchema', () => {
    for (const tool of SOLEIL_AI_REVIEW_ENGINE_TOOLS) {
  ...
  it('query tool requires "query" parameter', () => {
    const queryTool = SOLEIL_AI_REVIEW_ENGINE_TOOLS.find(t => t.name === 'query')!;
  ...
  it('cypher tool requires "query" parameter', () => {
    const cypherTool = SOLEIL_AI_REVIEW_ENGINE_TOOLS.find(t => t.name === 'cypher')!;
    expect(cypherTool.inputSchema.required).toContain('query');
  });

  it('context tool has no required parameters', () => {
    const contextTool = SOLEIL_AI_REVIEW_ENGINE_TOOLS.find(t => t.name === 'context')!;
    expect(contextTool.inputSchema.required).toEqual([]);
  });

  it('impact tool requires target and direction', () => {
    const impactTool = SOLEIL_AI_REVIEW_ENGINE_TOOLS.find(t => t.name === 'impact')!;
  ...
  it('rename tool requires new_name', () => {
    const renameTool = SOLEIL_AI_REVIEW_ENGINE_TOOLS.find(t => t.name === 'rename')!;
    expect(renameTool.inputSchema.required).toContain('new_name');
  });

  it('detect_changes tool has no required parameters', () => {
    const detectTool = SOLEIL_AI_REVIEW_ENGINE_TOOLS.find(t => t.name === 'detect_changes')!;
    expect(detectTool.inputSchema.required).toEqual([]);
  });

  it('list_repos tool has no parameters', () => {
    const listTool = SOLEIL_AI_REVIEW_ENGINE_TOOLS.find(t => t.name === 'list_repos')!;
