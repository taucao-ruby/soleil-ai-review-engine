...
Evaluate whether soleil-ai-review-engine code intelligence improves AI agent performance on real software engineering tasks. Runs SWE-bench instances across multiple models and compares baseline (no graph) vs soleil-ai-review-engine-enhanced configurations.
...
- Node.js 18+ (for soleil-ai-review-engine)
...
# soleil-ai-review-engine tool usage analysis
python -m analysis.analyze_results soleil-ai-review-engine-usage results/
...
eval/
  ...
  agents/
    soleil-ai-review-engine_agent.py      # soleil-ai-review-engineAgent: extends DefaultAgent with augmentation + metrics
  environments/
    soleil-ai-review-engine_docker.py     # Docker env with soleil-ai-review-engine + eval-server + standalone tool scripts
  bridge/
    soleil-ai-review-engine_tools.sh      # Bash wrappers (legacy — now standalone scripts are installed directly)
  ...
  prompts/
    ...
    system_native.jinja            # System: + soleil-ai-review-engine tool reference
    instance_native.jinja          # Instance: + soleil-ai-review-engine debugging workflow
    system_native_augment.jinja    # System: + soleil-ai-review-engine tools + grep enrichment docs
    instance_native_augment.jinja  # Instance: + soleil-ai-review-engine workflow + risk assessment
  configs/
...
2. **soleil-ai-review-engine setup**: Node.js + soleil-ai-review-engine installed, `soleil-ai-review-engine analyze` runs (or restores from cache)
3. **Eval-server starts**: `soleil-ai-review-engine eval-server` daemon (persistent HTTP server, keeps LadybugDB warm)
4. **Standalone tool scripts installed** in `/usr/local/bin/` — works with `subprocess.run` (no `.bashrc` needed)
5. Agent runs with the configured model + system prompt + soleil-ai-review-engine tools
6. Agent's patch is extracted as a git diff
7. Metrics collected: cost, tokens, tool calls, soleil-ai-review-engine usage, augmentation stats

### Tool architecture

```
Agent → bash command → /usr/local/bin/soleil-ai-review-engine-query
  → curl localhost:4848/tool/query     (fast path: eval-server, ~100ms)
  → npx soleil-ai-review-engine query                 (fallback: cold CLI, ~5-10s)
```
...
SWE-bench repos repeat (Django has 200+ instances at different commits). The harness caches soleil-ai-review-engine indexes per `(repo, commit)` hash in `~/.soleil-ai-review-engine-eval-cache/` to avoid redundant re-indexing.

### Grep augmentation (native_augment mode)

When the agent runs `grep` or `rg`, the observation is post-processed: the agent class calls `soleil-ai-review-engine-augment` on the search pattern and appends `[soleil-ai-review-engine]` annotations showing callers, callees, and execution flows for matched symbols. This mirrors the Claude Code / Cursor hook integration.
...
| GN Tool Calls | How many soleil-ai-review-engine tools the agent used |
