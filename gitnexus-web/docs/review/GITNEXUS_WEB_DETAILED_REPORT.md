# GitNexus-Web — Detailed Engineering Review

This document provides the full evidence-grounded analysis for each review phase. Cross-references to finding IDs (GNWEB-XXX) point to [GITNEXUS_WEB_FINDINGS_BACKLOG.md](./GITNEXUS_WEB_FINDINGS_BACKLOG.md).
...
2. **Server-backed mode** — Connect to GitNexus server → HTTP-backed Cypher, search, and file operations → chat
...
- 7 tools: `gitnexus_query` (semantic search), `gitnexus_cypher` (graph query), `gitnexus_grep` (regex file search), `gitnexus_read` (file reader), `gitnexus_overview` (codebase overview), `gitnexus_explore` (graph explorer), `gitnexus_impact` (blast radius analysis)
...
- All API keys stored as plaintext JSON in `localStorage` under key `gitnexus-llm-settings`
