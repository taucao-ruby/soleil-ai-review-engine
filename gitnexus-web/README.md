...
GitNexus-Web is a browser-based code intelligence tool for exploring repositories as knowledge graphs.
...
The package also includes a multi-provider LLM agent that can operate either against in-browser data or against a GitNexus server.
...
GitNexus-Web is a React 18 + Vite SPA with a heavy client-side execution model.
...
**Server-backed mode.**
The app connects to an existing GitNexus server and treats it as the source of graph data, search results, clusters, and process traces.
...
| Services | Reach GitHub or a GitNexus backend | `src/services/*` |
| External Systems | Hosting, Git provider, model provider, optional server | Vercel, GitHub, LLM APIs, GitNexus Server |
