#!/bin/bash
# soleil-ai-review-engine beforeShellExecution hook for Cursor
# Receives JSON on stdin with { command, cwd, timeout }
# Returns JSON on stdout with { permission, agent_message }
#
# Extracts search pattern from grep/rg commands, runs soleil-ai-review-engine augment,
...
# Run soleil-ai-review-engine augment
RESULT=$(npx -y soleil-ai-review-engine augment "$PATTERN" 2>/dev/null)
