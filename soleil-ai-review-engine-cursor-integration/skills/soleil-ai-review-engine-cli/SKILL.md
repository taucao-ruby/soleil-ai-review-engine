...
**When to run:** First time in a project, after major code changes, or when `soleil-ai-review-engine://repo/{name}/context` reports the index is stale.

### status — Check index freshness

```bash
npx -p soleil-engine-cli soleil status
```
...
```bash
npx -p soleil-engine-cli soleil clean
```
...
```bash
npx -p soleil-engine-cli soleil wiki
```
...
```bash
npx -p soleil-engine-cli soleil list
```
...
- **Index is stale after re-analyzing**: Restart Cursor to reload the MCP server
