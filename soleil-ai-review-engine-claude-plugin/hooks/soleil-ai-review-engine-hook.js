#!/usr/bin/env node
/**
 * soleil-ai-review-engine Claude Code Plugin Hook
 *
...
function runsoleil-ai-review-engineCli(args, cwd, timeout) {
  ...
  try {
    const which = spawnSync(
      isWin ? 'where' : 'which', ['soleil-ai-review-engine'],
  ...
  if (useDirectBinary) {
    return spawnSync(
      isWin ? 'soleil-ai-review-engine.cmd' : 'soleil-ai-review-engine', args,
      { encoding: 'utf-8', timeout, cwd, stdio: ['pipe', 'pipe', 'pipe'] }
    );
  }
  // npx fallback needs shell on Windows since npx is a .cmd script
  return spawnSync(
    isWin ? 'npx.cmd' : 'npx', ['-y', 'soleil-ai-review-engine', ...args],
